...
// Input orders to hidden form
document.getElementById('checkoutForm').addEventListener('submit', function() {
    var orderList = [];
    var orderRows = document.querySelectorAll('#order-list tr');
    
    orderRows.forEach(function(row) {
        var productId = row.getAttribute('data-id'); // Assuming each row has a 'data-id' attribute
        var productQuantity = parseFloat(row.cells[3].innerText); // Assuming quantity is in the fourth cell
